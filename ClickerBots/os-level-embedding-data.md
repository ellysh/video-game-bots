...
```
First line contains an [**include**](https://www.autoitscript.com/autoit3/docs/keywords/include.htm) keyword that allows you to append the specified file into the current script. **WinAPI.au3** file contains a defintion of the [**_WinAPI_GetClassName**](https://www.autoitscript.com/autoit3/docs/libfunctions/_WinAPI_GetClassName.htm) function that performs a necessary job. The script will sleep 5 seconds after the start that is performed by the [**Sleep**](https://www.autoitscript.com/autoit3/docs/functions/Sleep.htm) function. You should switch to the fullscreen window while the script sleeps. After sleep a handle of the current active window will be saved into the **$handle** variable. Last action is showing a message box by the [**MsgBox**](https://www.autoitscript.com/autoit3/docs/functions/MsgBox.htm) function with the necessary information.
